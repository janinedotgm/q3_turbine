...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        const { amount, playerNumber } = req.body;

        if (!amount || !playerNumber) {
            return res.status(400).json({ message: "Amount and player number are required" });
        }
        ...
        try {
            ...
            let tx = await program.methods
                .deposit(seed, new anchor.BN(amount))
                .accounts(accounts)
                .signers([playerKeypair, payer])
                .rpc();

            console.log('Deposit successful', tx);
            res.status(200).json({ message: "Deposit successful", tx });
